...
int main(int argc, char* argv[]) {
    ...
    // Default parameters
    std::string mesh_file = "./data/test.mesh";
    int order = 1;
    ...
    args.AddOption(&output_file, "-out", "--output", "Output file for solution.");
    ...
    try {
        mfem::Device device("cpu");  // or "hip", "occa", etc., depending on build
        device.Print();
